...
.. index::
  single: Journal of Machine Learning Research
...
.. meta::
  :description:
    提出在 LSTM 上增加 peephole connections
  :keywords:
    LSTM,
    RNN,
    Sequence Model,
...
.. tab-set::
  ...
  .. tab-item:: Tags
    ...
    :bdg-primary:`Journal of Machine Learning Research`
    :bdg-secondary:`LSTM`
    :bdg-secondary:`Model Architecture`
    :bdg-secondary:`Peephole Connections`
    :bdg-secondary:`RNN`
  ...
  .. tab-item:: Link
    ...
    論文連結 :footcite:`gers-etal-2002-learning`
...
- 作者認為在不給予 LSTM 模型任何的輸入時， LSTM 必須要能夠觀察 memory cell internal states 的變化才能模擬週期函數

  - 例如音樂節奏辨識

- 原版 LSTM :footcite:`hochreiter-etal-1997-long` 與 LSTM-2000 :footcite:`gers-etal-2000-learning` 都沒有 peephole connections

  - 此論文 :footcite:`gers-etal-2002-learning` 提議的 peephole connections 是只連接到相同的記憶單元
  - 現今常用的 LSTM 使用 peephole connections 的方法是全連接，例如 [PyTorch 實作的 LSTM][PyTorch-LSTM] 就是一個例子
  - 原版 LSTM :footcite:`hochreiter-etal-1997-long` 細節可以看[我的筆記][note-LSTM1997]
  - LSTM-2000 :footcite:`gers-etal-2000-learning` 細節可以看[我的筆記][note-LSTM2000]
...
..
  ...
  根據 LSTM-2000 :footcite:`gers-etal-2000-learning` 提出的架構如下（這篇論文不使用額外的**隱藏單元**，因此我們也完全不列出隱藏單元相關的公式）（細節可以參考[我的筆記][note-LSTM2000]）
  ...
  |$s_i^{\blk{k}}(t - 1)$|第 $t - 1$ 個時間點的第 $k$ 個**記憶單元區塊**的第 $i$ 個**memory cell internal states**|$i \in \set{1, \dots, \dblk}$|
  ...
  利用 $\eqref{1}$ 產生 $t$ 時間點的**memory cell internal states** $s^{\blk{k}}(t)$ 方法如下
  ...
  注意第 $k$ 個memory cell internal states**共享遺忘閘門** $y_k^{\opfg}(t)$ 與**輸入閘門** $y_k^{\opig}(t)$。
  ...
  產生 $t$ 時間點的**輸出**是透過 $t$ 時間點的**輸入**與**記憶單元輸出**（見 $\eqref{4}$）而得（注意是 $t$ 時間點不是 $t - 1$，代表原版 LSTM :footcite:`hochreiter-etal-1997-long` 與 LSTM-2000 :footcite:`gers-etal-2000-learning` 都寫錯了）
  ...
  - 針對前述問題提出的解決方法為 peephole connections
    - 所有閘門與memory cell internal states相接
  ...
  $\eqref{13}$ 的計算表示 $t$ 時間點的**遺忘閘門**與**輸入閘門**會與 $t - 1$ 時間點的**memory cell internal states相連**，並且閘門只會與對應的記憶單元連接。

  $\eqref{2}$ 的計算方法不變，在完成 $\eqref{2}$ 的計算後以 $t$ 時間點的memory cell internal states計算**輸出閘門**（注意不是 $t - 1$）：
  ...
  $\eqref{14}$ 的計算表示 $t$ 時間點的**輸出閘門**會與 $t$ 時間點的**memory cell internal states相連**，並且閘門只會與對應的記憶單元連接。
  ...
  |遺忘閘門偏差項初始值|$-2$|LSTM-2000 :footcite:`gers-etal-2000-learning` 採用的初始值為正數，這裡居然用負數|
  |輸入閘門偏差項初始值|$0$|原版 LSTM :footcite:`hochreiter-etal-1997-long` 採用的初始值為負數，這裡居然用 $0$|
  |輸出閘門偏差項初始值|$2$|原版 LSTM :footcite:`hochreiter-etal-1997-long` 採用的初始值為負數，這裡居然用正數|
  |參數初始化範圍|$[-0.1, 0.1]$||
  ...
  - 雖然 peephole connections 在這個任務中不重要，但仍然比 LSTM-2000 :footcite:`gers-etal-2000-learning` 表現還要好
  ...
  - 在 NMSD 任務中根據[圖 5](#paper-fig-5) 可以得到以下結論
    - 在此時驗中 LSTM-2000 :footcite:`gers-etal-2000-learning` 表現比 peephole connection 好
  ...
  圖 6：凸波偵測實驗中 LSTM-2000 :footcite:`gers-etal-2000-learning` 的計算狀態。
  圖片來源：[論文][論文]。
  ...
  - 透過實驗觀察發現 LSTM-2000 :footcite:`gers-etal-2000-learning` 學會兩種不同的方法進行凸波延遲偵測
    - LSTM-2000 :footcite:`gers-etal-2000-learning` 可以在每個時間點都增加memory cell internal states $s^{\blk{1}}$ 一點點，而預測值可以靠累加結果轉換而得（見[圖 6](#paper-fig-6) 左半）
    - LSTM-2000 :footcite:`gers-etal-2000-learning` 可以學會模擬振盪器，並根據振盪的次數進行預測（見[圖 6](#paper-fig-6) 右半）
  - 從[圖 6](#paper-fig-6) 的下半可以發現LSTM-2000 :footcite:`gers-etal-2000-learning` 的輸出閘門維持在 $1$ 的狀態
    - 作者認為由於預測行為很少發生，因此維持輸出並不會影響表現
    - 但當任務需要預測的頻率變高時，模型就必須只在適當的時間點開啟輸出閘門，而該行為在沒有 peephole connections 的狀況下無法達成（原始 LSTM 架構的輸出閘門只會獲得 $t - 1$ 時間點的計算狀態，並沒有 $t$ 時間點的memory cell internal states）
  - 從[圖 7](#paper-fig-7) 的下半可以發現加上 peephole connections 的 LSTM 會在大多數時間關閉輸出閘門
    - 由於新加入的機制比LSTM-2000 :footcite:`gers-etal-2000-learning` 更複雜，因此需要更多的時間才會收斂（見[圖 3](#paper-fig-3) 與[圖 5](#paper-fig-5)）
  ...
  - 由於 LSTM 在不直接觀察memory cell internal states的情況下無法完成 GTS（絕大多數的輸入都是 $0$），因此只顯示 peephole connections 的實驗
  ...
  - 根據[圖 9](#paper-fig-9) 下半我們可以發現輸出閘門只在需要生成凸波時開啟，生成完畢後馬上關閉
    - [圖 9](#paper-fig-9) 左下顯示生成凸波的當下由於遺忘閘門與輸入閘門一起關閉，因此memory cell internal states直接重設為 $0$
  - 作者嘗試在訓練時將遺忘閘門移除，發現模型無法收斂，證實遺忘閘門的必須性
  - 根據[圖 10](#paper-fig-10) 可以觀察到模型生成凸波的時間點跟memory cell internal states的增減時間相同
  ...
  - LSTM-2000 :footcite:`gers-etal-2000-learning` 只能模擬 $F = 10$ 的 $f_{\cos}$，且收斂時間長（見[圖 11](#paper-fig-11)）
  - 不使用遺忘閘門的原版 LSTM :footcite:`hochreiter-etal-1997-long` 無法模擬超過兩個以上的週期
  ...
  - 由於模型沒有收到任何輸入，完全只能依賴memory cell internal states進行模擬，因此memory cell internal states的數值變化應該要與模擬目標擁有類似的曲線（見[圖 13](#paper-fig-13)）
  - 作者認為此任務可以在不使用 peephole connections 的狀態下完成任務，但流經閘門的梯度被手動丟棄，因此 LSTM-2000 :footcite:`gers-etal-2000-learning` 的架構很難最佳化，導致實驗表現不佳（見[圖 11](#paper-fig-11)）
    - LSTM + peephole connections 收斂速度比 LSTM-2000 :footcite:`gers-etal-2000-learning` 快，見[圖 11](#paper-fig-11)
  ...
  - 從[圖 14](#paper-fig-14) 可以觀察到以下現象
    - 方波值為 $1$ 時
      - 記憶單元輸出與memory cell internal states的數值相同
      - 輸出閘門維持開啟
      - 模型內部狀態逐漸遞減（趨向 $0$）
      - 由於記憶單元輸出與記憶單元輸入連結的參數數值為負，因此模型有辦法遞減memory cell internal states
    - 方波值為 $0$ 時
      - 記憶單元輸出為 $0$
      - 輸出閘門維持關閉
      - 模型內部狀態逐漸遞增（趨向 $1$）
